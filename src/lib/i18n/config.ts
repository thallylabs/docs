...
export function normalizeI18nConfig(
  ...
  const defaultLocale =
    locales.find(
      (locale) => locale.code.toLowerCase() === requestedDefault?.toLowerCase(),
    )?.code ??
    locales.find(
      (locale) => locale.code.toLowerCase() === fallbackDefault?.toLowerCase(),
    )?.code ??
    locales[0].code
...
export function resolveI18nSelection(
  ...
  const sourceDefault = repository.locales.find(
    (locale) =>
  ...
  const locales = [
    ...
    ...normalized.locales.filter(
      (locale) =>
        locale.code.toLowerCase() !== sourceDefault.code.toLowerCase(),
    ),
  ].slice(0, 20)
...
export function localeDirection(code: string): 'ltr' | 'rtl' {
  ...
  return RTL_LANGUAGES.has(canonical.split('-')[0].toLowerCase())
    ? 'rtl'
    : 'ltr'
}
