...
description: Update branding, analytics, AI settings, and team access from the admin dashboard without rebuilding.
---
...
Set `THALLY_ANALYTICS_DB_URL` / `THALLY_ANALYTICS_DB_TOKEN` for traffic data, and `THALLY_DATABASE_URL` / `THALLY_DATABASE_TOKEN` for durable admin settings and related state. See the [environment variable reference](/guides/environment-variables#durable-storage-and-analytics).
