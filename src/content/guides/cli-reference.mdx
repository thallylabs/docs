...
Migrates a public documentation site or GitHub docs repository into a Thally project.

```bash
npx create-thally-docs migrate <github-or-docs-url> [output-dir] [options]
```

For a live URL, Thally discovers the sitemap, rendered navigation, redirects, and portable page content. For a GitHub URL, it clones the repository and reads Markdown, MDX, assets, and platform configuration statically. Both flows write a ready-to-run Thally project.

**Supported source platforms:** Mintlify, Docusaurus, GitBook, Nextra, VitePress, Starlight, and any repo with plain Markdown files.

<Note type="info" title="Use the source repository when possible">
Repository migration is the recommended auto-platform flow because it preserves authored navigation, assets, OpenAPI files, redirects, and MDX structure. A live website migration reconstructs only what the deployed site exposes and may need manual alignment.
</Note>
...
# Choose auto-detection interactively; the CLI will offer a GitHub repo first
...
- Live-site pages are discovered from sitemaps and rendered sidebars within the submitted docs path
...
| `--max-pages <count>` | Maximum live-site pages to import (`1`–`1000`) |
...
<Warning>
If interactive auto-detection receives a live URL, the CLI recommends its source GitHub repository. Continuing with the website requires confirmation that manual alignment may be needed and points you to the Thally MCP for refinement. Explicit `--platform auto` and `--yes` runs print the warning without adding a prompt.
</Warning>
...
| Docusaurus admonition | `<Note>` / `<Info>` / `<Warning>` / `<Error>` |
| Docusaurus tabs | `<Tabs>` and `<Tab>` |
| Rendered HTML table | Markdown table |
