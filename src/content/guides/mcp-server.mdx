...
Creates a fresh canonical Thally template, then imports a public docs site or GitHub docs repository. The target must be new or empty. This guarantees that the migrated project uses Thally's supported runtime, layout, navigation, and configuration instead of retaining the source application's shell.
...
| `projectDir` | Yes | Target path for the new canonical Thally project |
...
Imports content into an existing Thally project without scaffolding or replacing its runtime. Use this only when an in-place import is explicitly requested.

```
Import https://github.com/acme/api-docs into the existing Thally project at /workspace/docs
```

`import_docs` accepts the same source options as `migrate_docs`, but `projectDir` must point to an existing Thally project. It does not upgrade a legacy or third-party application to the current Thally template.
